...
**Hooks** are shell scripts that run automatically before tool use, after tool use, or when Claude stops. By default, only `block-dangerous-commands.sh` is active — it blocks destructive shell commands and prompts on sensitive file access. Additional hooks for branch protection, auto-formatting, auto-linting, and completion notifications are available in `.claude/hooks/` and can be enabled in `.claude/settings.json`.
...
User → Command → (reads Rules) → delegates to Subagents → (Skills activate on-demand)
                                            ...
                                            (PostToolUse, Stop: opt-in hooks available)
```
